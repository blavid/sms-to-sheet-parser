...
// Global mock for Pipedream's $ context
const mockExport = jest.fn();
global.$ = { export: mockExport };

// Mock defineComponent so we can call parser.run directly
jest.mock('../src/index.js', () => {
  const actual = jest.requireActual('../src/index.js');
  return {
    __esModule: true,
    default: {
      run: actual.default,  // the real run function
    },
  };
});
...
describe('SMS Transaction Parser', () => {
  ...
  const runParser = async (smsText) => {
    ...
    await parser.run({ steps, $: { export: mockExport } });
    return mockExport.mock.calls[0]?.[1]; // returns the exported rows
  };
...
***5267 had a transaction of ($90.00). Description: POS Transaction ZIPLY FIBER INTERNET 135 Lake Street South SKIRKLAND WAU. Date: Nov 09, 2025 `;
    ...
    expect(rows[01]).toBe('POS Transaction ZIPLY FIBER INTERNET 135 Lake Street South SKIRKLAND WAU');
  });

  test('Unknown message → Manual Review Required', async () => {
    const rows = await runParser(`Some random SMS with $42.00 in it`);
    expect(rows[ Heisenberg[0][1]).toBe('Manual Review Required');
