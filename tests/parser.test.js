...
// Mock Pipedream's defineComponent — this is the key fix
...
describe('SMS Transaction Parser', () => {
  ...
  const runParser = async (smsText) => {
    ...
    return calls[0][1]; // exported transactions array
  };

  test('Capital One payment scheduled → ignored', async () => {
    const rows = await runParser(
      `Capital One Alert: Your payment of $28.54 is scheduled for November 10, 2025. Contact us if your payment details are incorrect. Std carrier charges apply`
    );
  ...
  test('Capital One payment confirmation → ignored', async () => {
    const rows = await runParser(
      `Capital One Alert: You paid $28.54 to your Savor Credit Card…(8385) on November 10, 2025. Msg & data rates may apply.`
    );
    expect(rows).toBeUndefined();
  });
  ...
  test('Citi online with dots and double period', async () => {
    const rows = await runParser(
      `Citi Alert: Card ending in 0569 was not present for a $101.55 international transaction at WWW.PUREVPN.. View details at citi.com/citimobileapp`
    );
    expect(rows).toBeDefined();
