...
{
  // Citi Alerts (Online) - FINAL robust version
  regex: /Citi Alert: Card ending in \d{4} was not present for a \$(\d+\.\d{2}) (international )?transaction at\s+([A-Z0-9][A-Z0-9.*-]*?(?:\.[A-Z0-9.*-]+)*?)(?:\.\.| \.|$|\s+View|\s+citi)/i,
...
},
{
  ...
  extract: (match) => ({
    ...
    let matched = false;
    // First Tech CU Multi-Transaction: Split and parse each transaction
    if (smsText.includes('Transaction Alert from First Tech Federal Credit Union')) {
      // Split by '***' delimiter for multiple transactions
      const transactionBlocks = smsText.split('***').filter(block => block.trim());
      for (const block of transactionBlocks) {
        const match = block.match(/had a transaction of \(\$(\d+\.\d{2})\)\. Description: (.+?)\. Date: (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) (\d{1,2}), (\d{4})/);
        if (match) {
          const payee = match[2].trim();
          // Skip excluded payees
          if (!excludedPayees.includes(payee)) {
            // Set paymentMethod to 'Venmo' if payee is 'ACH Debit VENMO - PAYMENT'
            const paymentMethod = payee === 'ACH Debit VENMO - PAYMENT' ? 'Venmo' : 'Debit Card';
            transactions.push({
              date: new Date(`${match[3]} ${match[4]}, ${match[5]}`).toLocaleDateString('en-US', {
                month: '2-digit',
                day: '2-digit',
                year: 'numeric'
              }), // Use SMS date as MM/DD/YYYY
              amount: match[1],
              payee: payee,
